...
_VOCAB_RULES = (
    ...
    "  ★三個干擾項必須是『同一個詞的 plausible 誤讀』，至少要和正答共享部分假名聲音；"
    "不可塞完全不相干詞的讀音，也不可塞整句/片語長度的假名串。\n"
    "  ★鐵則：被考的〈〉詞『必須至少含一個漢字』。純假名／純片假名詞（如「メルト」"
    "「ありがとう」）絕對不可當考點——那沒有漢字可讀，不成題；若素材整句都沒有合適"
    "的含漢字 N1 詞，請改自選一個契合作品的含漢字 N1 詞，或改出其他兩種題型。\n"
    "  ★嚴禁直接考歌名／標題怎麼讀。像「次の曲名『X』の読み方」這種題一律不准；"
    "漢字読み必須考可獨立記住、可脫離標題使用的詞彙項目。\n"
    "\n"
    ...
    "2.1 題幹與例句必須有『記憶價值』：讀完要能幫助記住該考點。禁止只靠 stock template "
    "包裝、沒有語境支撐的空泛句子。\n"
