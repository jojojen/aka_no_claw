...
def parse_chat_tool_plan(raw: object) -> ChatToolPlan | None:
    ...
    - ``{"tool":"__no_tool__"}`` for the hidden decision-only path
    ...
    if tool == CHAT_TOOL_NO_TOOL:
        ...
        # is always generated by the dedicated answer stage.
...
@dataclass(frozen=True, slots=True)
class ChatEvidence:
    """A durable, domain-neutral observation produced by a tool execution.

    ``source_type=tool_result`` is an observed tool result. Other source types
    (for example ``derived_answer``) remain replayable but are explicitly not
    promoted to evidence. ``summary`` is only a compact navigation aid.
    """
...
_CHAT_SYSTEM_PROMPT = (
    ...
    "並以繁體中文自然作答。對話中較早的助理回答只是未驗證的先前說法，不是權威事實；"
    "若使用者後來補充、更正或否定限制，必須以較新的使用者訊息為準。"
    "工具證據與使用者陳述會被清楚標示；不要把助理先前的猜測提升成事實。"
    "外部可驗證的數字或事實若沒有使用者陳述或工具證據支持，必須明確標成假設／估計，"
    "或直接說目前無法確認，不得自行補造來源、價格、費用、期間或市場狀態。"
)
    ...
    *,
    ...
    for turn in history:
        label = (
            ...
            else _CHAT_ROLE_LABELS.get(turn.role, turn.role)
        )
