...
CODEGEN_SEED: tuple[dict, ...] = (
    ...
    {
        "category": "output_contract",
        "title": "先保存語義內容，再由格式渲染器產生下載檔",
        "technique": (
            "需要同一份結果輸出成多種檔案格式時，不要把回答文字直接改副檔名，也不要讓"
            "資料擷取或業務分支各自拼檔案。先保留一份完整的語義內容，再交給由格式名稱"
            "選擇的渲染器產生 MIME、檔名與完整文件。每個格式測試必須驗證標題、表格、"
            "連結與文字仍存在，不能只驗證檔案建立成功。對可執行的文件格式，還要在下載"
            "前移除主動內容，並用 attachment 與 nosniff 回應標頭降低執行風險。"
        ),
        "keywords": [
            "*",
            "artifact",
    ...
    {
        "category": "concurrency",
        "title": "初始化檢查與首次寫入必須共用同一個鎖",
        "technique": (
            "延遲初始化持久化資料時，不要分開執行空值檢查與首次寫入。多個請求可同時看見"
            "空狀態，並重複寫入相同的第一個序號。用同一個鎖包住讀取、遷移與首次寫入。"
            "第二個請求取得鎖後必須重新讀取狀態。用並行測試驗證初始化只執行一次，且所有"
            "呼叫者讀到相同的有效序列。"
        ),
        "keywords": [
            "*", "initialization", "migration", "journal", "sequence", "lock",
            "concurrency", "race", "初始化", "遷移", "序列", "鎖", "競態",
        ],
        "confidence": 0.95,
    },
    {
        "category": "reliability",
        "title": "先持久化完整結果，再交給可中斷的傳輸層",
        "technique": (
            "長任務不能等到串流產生器送出終止事件時才建立附件或保存結果。用戶端可在任務"
            "完成前關閉連線，導致傳輸包裝器不再執行。工作執行緒必須先產生最終文字與附件，"
            "再把同一份引用交給串流。若連線已中斷，將文字與附件引用寫入原工作階段。測試"
            "必須關閉串流、讓背景工作完成、重新載入同一工作階段，並實際開啟附件。"
        ),
        "keywords": [
            "*", "stream", "disconnect", "artifact", "persistence", "session",
            "background", "串流", "斷線", "附件", "持久化", "工作階段",
        ],
        "confidence": 0.95,
    },
)
