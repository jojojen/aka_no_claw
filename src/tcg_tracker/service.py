...
class TcgPriceService:
    def __init__(
        ...
        *,
        ...
        resolved_db_path = Path(db_path or settings.monitor_db_path)
        self._log_raw_result_limit = settings.log_raw_result_limit

        self.database = MonitorDatabase(resolved_db_path)
        self.database.bootstrap()
        primary_client = yuyutei_client or YuyuteiClient(
            HttpClient(user_agent=settings.yuyutei_user_agent)
        )
        if reference_clients is not None:
            ...
            shared_http_client = HttpClient(user_agent=settings.yuyutei_user_agent)
