...
def test_research_shows_out_of_stock_raw_sell_as_weak_upper_bound() -> None:
    """#41: a sold-out raw card (在庫×) still has a 販売 price worth showing. It is
    surfaced as a weak「在庫なし」upper bound alongside 買取, not dropped."""
    def shop_reference(query: str, price_cap: int) -> ShopReference:
        return ShopReference(
            label="遊々亭",
            buy_reference=9000,
            sell_reference=None,          # nothing purchasable
            stock_total=0,
            buy_count=1,
            sell_count=0,
            sample_urls=("https://yuyu-tei.jp/sell/ws/card/pjs2.0/10208",),
            buy_min=9000,
            buy_max=9000,
            oos_sell_reference=14800,
            oos_sell_min=14800,
            oos_sell_max=14800,
            oos_sell_count=1,
        )
    ...
    reply = handler("風に舞う花びらの中で 初音ミク SSP", "chat-shop-rawoos")

    assert "買取¥9,000／販売¥14,800（在庫なし・上限參考弱）" in reply


def test_research_labels_yuyutei_raw_in_graded_context() -> None:
    """#41: in a PSA10/graded query the 遊々亭 band is labeled「raw参考」so it is
    never mistaken for a graded fair-market comp."""
    def shop_reference(query: str, price_cap: int) -> ShopReference:
        return ShopReference(
            label="遊々亭",
            buy_reference=9000,
            sell_reference=14800,
            stock_total=2,
            buy_count=1,
            sell_count=1,
            sample_urls=("https://yuyu-tei.jp/sell/ws/card/pjs2.0/10208",),
    ...
    reply = handler("風に舞う花びらの中で 初音ミク SSP PSA10", "chat-shop-graded")

    assert "遊々亭 raw参考帯 買取¥9,000／販売¥14,800（在庫2点）" in reply


def test_yuyu_raw_card_search_term_strips_grading_then_normalizes() -> None:
    """#41 root cause: the noisy price query (grading + platform noise) makes the
    遊々亭 search_word 0-hit. Grading tokens are stripped via the closed enum, then
    the LLM normalizer is handed the grading-free text to derive the raw card."""
    from openclaw_adapter import research_command as rc
    ...
    def normalizer(text: str) -> str:
        seen.append(text)
        return "風に舞う花びらの中で 初音ミク SSP"

    term = rc._yuyu_raw_card_search_term(
        "風に舞う花びらの中で 初音ミク ssp PSA10 プロセカ", normalizer
    )
    # Grading token (PSA10) is gone before the normalizer ever sees the text.
    assert "PSA10" not in seen[0]
    assert term == "風に舞う花びらの中で 初音ミク SSP"


def test_yuyu_raw_card_search_term_falls_back_when_normalizer_unsure() -> None:
    """When the LLM normalizer returns None (unsure), the grading-stripped query is
    used — never the original noisy one with the grading token."""
    from openclaw_adapter import research_command as rc

    term = rc._yuyu_raw_card_search_term(
        "初音ミク SSP PSA10", lambda text: None
    )
    assert "PSA10" not in term
...
def test_shop_reference_from_band_reports_zero_hits_as_no_match(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        ...
        lambda *a, **kw: {"__no_data__": True, "reason": "no_match", "diag": {}},
    )

    with pytest.raises(rc.ShopReferenceUnavailable) as excinfo:
        rc._shop_reference_from_band("query", 5000, None)
    assert excinfo.value.reason == "no_match"


def test_price_section_shows_distinct_reason_for_no_match() -> None:
    """The no_match reason renders a 'queried but found nothing' note, distinct
    from the cooldown 'rate-limited' note (#41)."""
    ...
    result = _build_price_section_result(
        ...
        shop_reference=None,
        shop_reference_reason="no_match",
    )
    assert "查無對應卡" in result.summary
    assert "rate-limited" not in result.summary
