...
| Web output artifacts | `tests/test_artifact_store.py`, artifact tests in `tests/test_command_bridge.py` and `tests/test_session_projection.py`, Web `ArtifactList` and session tests | Supported restart; produce one PDF and one image, reload the page, open both files, and confirm a different session cannot fetch either file |
