...
| [WEB_CONVERSATION_COMPACTION_IMPLEMENTATION_PLAN.md](WEB_CONVERSATION_COMPACTION_IMPLEMENTATION_PLAN.md) | Current | command-bridge / model-context | Grounded, versioned context checkpoints that preserve the authoritative event history (issue #87). Depends on the event spine. |
| [WEB_ARTIFACT_DELIVERY_IMPLEMENTATION_PLAN.md](WEB_ARTIFACT_DELIVERY_IMPLEMENTATION_PLAN.md) | Current | command-bridge / conversation-runtime | Canonical contract for durable Markdown, CSV, PDF, and image results in the Web console. |
