...
- 文法卡的 `explanation_zh` 是「預習用短摘要」，不是題目揭示頁的完整解說。不得回填 `正しい並びは...`、`★印...`、`正解...`、`【読み】...` 這類解題資訊。
- 文法卡說明採 deterministic/manual 規則，不靠本地模型批次改寫品質。請以 `src/openclaw_adapter/grammar_card_manual_overrides.py` 為準；明確 override、headword 模板與結構模板都在那裡。
- 文法卡例句採 fail-closed：只接受填空題題幹加正解生成的 `adapted` 完整句，或明確人工審過的 `adapted` 例句。`adapted` 卡片來源一律標 `改寫例句`，不可沿用歌曲 URL 讓使用者誤以為是原文。
- `文の組み立て` 題目除非有人工審過的完整改寫例句，否則不得自動生成文法卡。評論、考察、來源片段、排序答案碎片都只留在題庫，不進 learner-facing 卡片。
- 詞彙題也採同樣標準：不得用沒有記憶價值的 stock template 包題。尤其禁止兩類：
  - `漢字読み` 直接問歌名／標題怎麼讀。
  - `用法` 只用 `次のうち、語句「X」の使い方として最も適切な文はどれか。` 這類泛用外殼包四個句子。
- `漢字読み` 的干擾讀音也有硬規則：必須是**同一個目標詞**的 plausible 誤讀。與正答完全不共享任何假名、或長到像片語讀音的選項，直接判定為垃圾題，不准入庫。
- 詞彙題幹也不准拿 `解説によれば`、`筆者は読み解いている`、`語り手` 這類後設評論文來包單字題。這不是在教詞，而是在考解說文。
- 補充界線：如果某個詞在**正常歌詞句子**裡自然出現，而它剛好也是歌名的一部分，這是可以的。禁止的是「把標題本身當題目」；不是禁止那個詞出現在真實句子裡。
- 所有題目、選項、卡片例句都必須有**記憶價值**：讀完要能幫助學習者記住該考點。只求形式可作答、但句子空泛、荒謬、或只是模板替換，不合格。
...
7. **記憶價值**：題幹本身、正解句、干擾句都要能支撐記憶。禁止：
   - 只是在考歌名或標題讀音。
   - 泛用模板 stem 加上機械替換選項。
   - 荒謬或明顯亂造、只為了湊四選一的句子。
   - 沒有完整語境、無助於記住單字/文法的碎片。
   - 但若歌名中的詞彙同時出現在真實歌詞句子裡，且句子本身有語境與記憶價值，則可用該句子出題。
...
題庫與知識庫路徑：runtime 一律取 `settings.quiz_db_path`，設定來源是
`src/assistant_runtime/settings.py` 的 `OPENCLAW_QUIZ_DB_PATH`，預設值為
`data/quiz.sqlite3`。`data/quiz.sqlite` 目前是重構後殘留的空檔，不是 live 題庫。
