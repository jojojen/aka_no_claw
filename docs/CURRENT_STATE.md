...
| Web command bridge | `shipped` | `python -m openclaw_adapter command-bridge --lan --port 8781`, `/api/command`, `/api/command/stream`, `/api/command/async`, `/api/command/events`, `/api/command/session`, `/api/command/approval` | `.openclaw_tmp/web_sessions/` authoritative event journals; `.openclaw_tmp/web_approvals/` pending/resolved approval records | The #84 event spine remains authoritative. Issue #85 adds a locally enabled, manifest-bound approval gate for generated workflow tools: risky effects pause before execution, approve-once is single-use, and reject/expiry/restart/hash drift fail closed. |
