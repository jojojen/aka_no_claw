...
| Web command bridge | `shipped` | `python -m openclaw_adapter command-bridge --lan --port 8781`, `/api/command`, `/api/command/stream`, `/api/command/async`, `/api/command/events`, `/api/command/session`, `/api/command/approval`, `/api/command/queue`, `/api/command/artifacts/{id}/{filename}` | `.openclaw_tmp/web_sessions/` authoritative event journals; `.openclaw_tmp/web_approvals/` approval records; `.openclaw_tmp/web_prompt_queue/` bounded queue snapshots; `.openclaw_tmp/web_artifacts/` session-bound output files | The #84 event spine remains authoritative. #85 adds the manifest-bound approval gate. #86 adds a durable prompt queue. The artifact contract returns durable Markdown, CSV, PDF, and image references. |
...
| Dynamic tools `/new` | `beta` | Telegram `/new`, Web workflow generated-tool steps, `openclaw_adapter.dynamic_tools` selftest | `generated_tools/`, `knowledge.sqlite3` | Local Ollama by default; OpenCode Big Pickle codegen can be enabled. Web workflow execution classifies effects from the frozen artifact and requires one-shot approval for risky actions; Telegram behavior is unchanged. |
