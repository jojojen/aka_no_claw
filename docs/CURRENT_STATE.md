...
| Web command bridge | `shipped` | `python -m openclaw_adapter command-bridge --lan --port 8781`, `/api/command`, `/api/command/stream`, `/api/command/async`, `/api/command/events`, `/api/command/session`, `/api/command/approval`, `/api/command/queue` | `.openclaw_tmp/web_sessions/` authoritative event journals; `.openclaw_tmp/web_approvals/` approval records; `.openclaw_tmp/web_prompt_queue/` bounded queue snapshots | The #84 event spine remains authoritative. #85 adds the manifest-bound approval gate. #86 adds an opt-in (`OPENCLAW_WEB_PROMPT_QUEUE_ENABLED`) durable per-session next-turn queue with versioned edit/cancel/reorder, restart recovery, and goal-loop-safe interjections. |
