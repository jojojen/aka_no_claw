...
完整邏輯寫在 [docs/LIQUIDITY_METHODOLOGY.md](docs/LIQUIDITY_METHODOLOGY.md)。
...
如果你想直接雙擊或用一行命令啟動，也可以用 `launchers/` 資料夾內的批次檔：

```powershell
launchers\start-dashboard.bat
```

要改 port 或 host 時，參數會直接透傳：

```powershell
launchers\start-dashboard.bat --host 127.0.0.1 --port 8766
```
...
- 如果 `REPUTATION_AGENT_SERVER_URL` 指向 `127.0.0.1` 或 `localhost`，但本機 `reputation_snapshot` 還沒啟動，bot 會嘗試自動拉起隔壁 repo 的 `reputation_snapshot/launchers/start.bat`。
...
```powershell
launchers\start-telegram-bot.bat --notify-startup --with-reputation-agent
```
...
```powershell
launchers\start-dashboard.bat --with-reputation-agent
```
...
- `launchers\start-dashboard.bat` 也會在啟動前自動同步 `.venv` 依賴，並確認 Playwright Chromium 已安裝。
...
這個來源目錄目前定義在 [config/reference_sources.json](config/reference_sources.json)，之後不同模組都可以共用這份來源權重與角色設定。
